"""
Variant 5 of FINd: vectorised index computation in decimateFloat.
...
class FINDHasher_v5_decimate:
	...
	def decimateFloat(cls, in_, inNumRows, inNumCols, out):
		# Vectorised index computation — same formula as original:
		...
		# Reshape flat input to 2D using the ORIGINAL's indexing
		# convention: in_[ini * inNumCols + inj]
		arr = np.array(in_, dtype=np.float64).reshape(-1, inNumCols)

		# Fancy indexing: for each (i,j), take arr[row_idx[i], col_idx[j]]
		sampled = arr[np.ix_(row_idx, col_idx)]

		# Write back into the nested-list output
		for i in range(64):
			for j in range(64):
