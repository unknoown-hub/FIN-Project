"""
Variant 1 of FINd: vectorised RGB-to-luminance conversion.
...
class FINDHasher_v1_luminance:
	...
	def fillFloatLumaFromBufferImage(self, img, luma):
		...
		# Read the full image into a numRows x numCols x 3 array of
		# float64. This replaces 3 * numRows * numCols calls to
		# PIL.getpixel with a single C-level buffer read.
		arr = np.asarray(rgb_image, dtype=np.float64)

		# Vectorised luminance: same coefficients, same
		# float64 arithmetic, applied across the whole array at once.
		luma_2d = (
