"""
Variant 6 of FINd: NumPy matrix multiplication replaces triple nested loops in dct64To16.
...
class FINDHasher_v6_dct:
	...
	def dct64To16(self, A, T, B):
		...
		# B = D A Dᵀ, computed as two matrix multiplications
		T_arr = D @ A_arr                                  # 16x64
		B_arr = T_arr @ D.T                                # 16x16

		# Write back into the nested-list output structure
		for i in range(16):
			for j in range(16):
				B[i][j] = float(B_arr[i, j])

		# Also populate T for interface compatibility (original
		# writes to it even though downstream doesn't use it)
		for i in range(16):
			for j in range(64):
