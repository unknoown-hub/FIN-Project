"""
Variant 3 of FINd: np.median replaces the custom Torben algorithm in dctOutput2hash.
...
class FINDHasher_v3_median:
	...
	def dctOutput2hash(self, dctOutput16x16):
		...
		dctMedian = float(np.median(np.array(dctOutput16x16,
		                                     dtype=np.float64)))
		for i in range(16):
			for j in range(16):
