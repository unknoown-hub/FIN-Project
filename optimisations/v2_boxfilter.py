"""
Variant 2 of FINd: vectorised box filter via summed-area table (integral image).
...
class FINDHasher_v2_boxfilter:
	...
	def boxFilter(cls, input, output, rows, cols, rowWin, colWin):
		...
		# Reconstruct the 2D luminance array from the row-major flat list.
		# The original uses `input[k*rows + l]` — this is a bug in the
		# original (should be `k*cols + l`), but we must preserve it
		# exactly to match behaviour.
		arr = np.array(input, dtype=np.float64)
		# Reshape faithfully to the original's indexing pattern:
		# original reads input[k*rows + l] for k in [xmin,xmax),
		# l in [ymin,ymax). This implies the original treats `input`
		# as having stride `rows`, not `cols`. Preserve this exactly.
		arr2d = arr.reshape(-1, rows)  # stride = rows, matches original

		# Build integral image with zero-padding on top/left
		padded = np.zeros((arr2d.shape[0] + 1, arr2d.shape[1] + 1),
		                  dtype=np.float64)
		padded[1:, 1:] = arr2d
		integral = padded.cumsum(axis=0).cumsum(axis=1)

		# Compute each output pixel's window sum and clipped area
		...
		# Broadcast to compute output[i,j] for all (i,j) at once
		...
		# Write back into the flat output list, preserving the
		# original's row-major indexing (stride = rows)
		flat = result.ravel()
